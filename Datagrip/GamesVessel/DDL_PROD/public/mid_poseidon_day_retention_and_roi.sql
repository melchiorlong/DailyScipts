...
(
	...
	constraint mid_poseidon_day_retention_and_roi_pkey
		primary key (app_name, country, date, team, optimizer, media_source)
)
sortkey(app_name, country, date);

alter table public.mid_poseidon_day_retention_and_roi owner to gv_root;
...
grant select on public.mid_poseidon_day_retention_and_roi to metabase;

grant select on public.mid_poseidon_day_retention_and_roi to gv_developer;
