...
class applovin_hint_calc:
    ...
    def get_result_csv(self, rowset_list):
        ...
        with open(filepath, 'w') as file:
            file.write(','.join(['trigger_type', 'app_name', 'country', 'sum_dnu', 'persent']))
            file.write('\n')
            for rs in rowset_list:
                for r in rs:
                    ...
                    data = ','.join([str(trigger_type), app_name, country, str(dnu_sum), str(persent)])
                    file.write(data)
                    file.write('\n')
