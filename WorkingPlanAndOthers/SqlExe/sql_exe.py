...
date_list = get_date_range('2022-04-14', '2022-04-21')
...
for exe_date in date_list:
    sql_statement = """
insert into temp_ads_mkt_mediation_monitoring_20220422 (country,
                                                        app_name,
                                                        placement_name,
                                                        rev_bj_date,
                                                        version,
...
with psd_log_all           as (
    select
        ...
        case
            when (app_name = 'saori_ip' and app_version_code >= 29)
                then 'Max'

            when (app_name = 'saori_ip' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'saori_gp' and app_version_code >= 43)
                then 'Max'

            when (app_name = 'saori_gp' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'saga_ip' and app_version_code >= 999)
                then 'Max'

            when (app_name = 'saga_ip' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'saga_gp' and app_version_code >= 999)
                then 'Max'

            when (app_name = 'saga_gp' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'dohko_ip' and app_version_code >= 47)
                then 'Max'

            when (app_name = 'dohko_ip' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'dohko_gp' and app_version_code >= 54)
                then 'Admob'

            when (app_name = 'dohko_gp' and app_version_code >= 51)
                then 'Max'

            when (app_name = 'dohko_gp' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'aiolos_ip' and app_version_code >= 24)
                then 'Max'

            when (app_name = 'aiolos_ip' and app_version_code >= 0)
                then 'Mopub'

            when (app_name = 'aiolos_gp' and app_version_code >= 21)
                then 'Max'

            when (app_name = 'aiolos_gp' and app_version_code >= 20)
                then 'Admob'

            when (app_name = 'aiolos_gp' and app_version_code >= 0)
                then 'Mopub'
                else 'Mopub' end                           as version,
        ...
        case
            when vendor = 'facebook'
                and lower(ilrd_mediation_type) <> 2
        ...
        case
            when
                    ...
                    and lower(ilrd_mediation_type) <> 2
                then 0
            when vendor = 'innerpromote'
                then 0
                else ilrd_publisher_revenue
